...
public class Director {
	...
	public boolean setFormations(BattleFields BF,int x, int y,int size,String formationName, Vector<Creature> list) {
		int length = 0, height = 0;
		boolean[][] subGraph;
		switch(formationName) {
		case "鹤翼"://坐标为左上
			length = list.size();
			height = list.size()/2 + list.size()%2;
			subGraph = new boolean[length][height];
			...
			for(int i = 0; i < length/2;i++)
				subGraph[length - 1 - i][i] = true;
			if(BF.Containable(x, y, length, height, subGraph)) {
		...
		case "鱼鳞"://坐标为能包含鱼鳞的最小矩形左上
			int upSize = list.size()-1;
			int count = 0;
			for(int i = 1; count < upSize;i++)
				count += i;
			length = count;
			height = count + 1;
			subGraph= new boolean[length][height];
			...
			for(int i = 0; i < count;i++) {
				int start = (length - 1 - i) / 2;
				for(int j = ; j <= i; j++)
					subGraph[i][start + j]=true;
			}
			if(BF.Containable(x, y, length, height, subGraph)) {
