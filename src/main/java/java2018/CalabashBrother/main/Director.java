...
public class Director {
	...
	public Director(BattleFields bfs){
		BFs = bfs;
	}
	
	public Director(BattleFields bfs, CalabashBrothers cbs){//, Monsters ms){
		BFs = bfs;
		CBs = cbs;
		//Ms =ms;
	}
	...
	public void setPos() {
		...
		CBList.add(getBrother(0));
		CBList.add(getBrother(1));
		CBList.add(getBrother(2));
		CBList.add(getBrother(3));
		CBList.add(getBrother(4));
		CBList.add(getBrother(5));
		CBList.add(getBrother(6));
		do {
		...
		*/
		...
		}while(!setFormation(x1, y1, 10, formationName, MonsterList));
		//放置蛇精
